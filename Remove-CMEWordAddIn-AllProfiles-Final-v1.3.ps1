...
param(
    $SkipProfiles = @(
        ...
        'zero','Administrator','VASCANSVCert'
    ),
    $NamePatterns = @(
        'CMEWordAddIn',
        'WordAddIn1',
...
function Invoke-RegCommand {
    ...
    try {
        $p = Start-Process -FilePath 'reg.exe' -ArgumentList $Arguments -Wait -PassThru -WindowStyle Hidden -RedirectStandardOutput $outFile -RedirectStandardError $errFile
        $output = @()
...
function Remove-MatchingSubKeys {
    ...
    $removed = 0
    if (-not (Test-Path -LiteralPath $BasePath)) { return $removed }
    ...
    foreach ($child in @(Get-ChildItem -LiteralPath $BasePath -ErrorAction SilentlyContinue)) {
        ...
        $hit = Test-PatternMatch -Text $child.PSChildName -Patterns $Patterns

        if (-not $hit -and $props) {
            foreach ($propName in $PropertyNames) {
                $value = $props.$propName
                if (Test-PatternMatch -Text ([string]$value) -Patterns $Patterns) {
                    $hit = $true
                    break
                }
            }
        ...
        if ($hit) {
            if (Remove-KeyTree -Path $child.PSPath -Reason $Reason) { $removed++ }
        }
    }
...
function Remove-WordAddinKeys {
    ...
    foreach ($path in $paths) {
        $removed += Remove-MatchingSubKeys -BasePath $path -Reason 'Word add-in key' -Patterns $Patterns -PropertyNames @('Manifest','FriendlyName','Description')
    }
...
function Remove-UninstallKeys {
    ...
    return (Remove-MatchingSubKeys -BasePath $path -Reason 'HKCU uninstall key' -Patterns $Patterns -PropertyNames @('DisplayName','Publisher','UninstallString','UrlUpdateInfo','DisplayVersion'))
}
...
function Remove-ResiliencyKeys {
    param(
        [string]$Root,
        $Patterns
    )
    ...
    $removed = 0
    $paths = @(
        (Join-Path $Root 'Software\Microsoft\Office\Word\Resiliency'),
        (Join-Path $Root 'Software\Microsoft\Office\16.0\Word\Resiliency')
    )
        ...
        foreach ($child in @(Get-ChildItem -LiteralPath $path -ErrorAction SilentlyContinue)) {
            ...
            if (-not $hit -and $props) {
                foreach ($p in $props.PSObject.Properties) {
                    if (Test-PatternMatch -Text ([string]$p.Value) -Patterns $Patterns) {
                        $hit = $true
                        break
                    }
                }
            ...
            if ($hit) {
                if (Remove-KeyTree -Path $child.PSPath -Reason 'Word resiliency key') { $removed++ }
            }
        }
    }
...
function Invoke-ProfileCleanup {
    ...
    $resiliencyKeys = Remove-ResiliencyKeys -Root $Root -Patterns $Patterns
    $folders = Remove-InstallFolder -ProfilePath $ProfilePath -RelativePath $RelativeInstallFolder

    Write-Log "Summary for ${ProfileName}: WordAddins=${wordAddins} UninstallKeys=${uninstallKeys} VSTOInclusion=${vstoKeys} Resiliency=${resiliencyKeys} InstallFolders=${folders}"
}
...
foreach ($item in @(Get-ChildItem -LiteralPath $profileRoot -ErrorAction Stop)) {
    try {
        ...
        if ($SkipProfiles -icontains $profileName) { continue }
